...
1. **Rotate now:** Supabase Dashboard → Account → Access Tokens → revoke `***REMOVED***` → generate a new one → put it in your local `.cursor/mcp.json` (now gitignored, won't be committed).
