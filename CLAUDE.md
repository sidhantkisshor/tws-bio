...
tws.bio is a URL shortener with analytics and mobile deep linking, built with Next.js 15 (App Router), React 19, TypeScript 5, Tailwind CSS v4, and Supabase (PostgreSQL + Auth).
...
- `src/lib/supabase/middleware.ts` — Middleware client (operates on `NextRequest`), only used in `src/middleware.ts`

### Rendering Model

- Most pages are Client Components (`'use client'`): home, login, signup
- `dashboard/page.tsx` is a Server Component — auth guard via `redirect('/login')`, fetches data server-side
- `src/app/[shortCode]/route.ts` is a Route Handler (not a page) — the core redirect logic

### Key Data Flow

1. **Link creation**: Client calls `create_link` Supabase RPC (SECURITY DEFINER, allows anonymous creation with null `user_id`)
...
- Several npm packages are installed but unused (react-icons, recharts, qrcode, nanoid, uuid, date-fns)
