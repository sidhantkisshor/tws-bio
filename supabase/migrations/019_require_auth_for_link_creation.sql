-- Migration 019: Require authentication to create links
--
-- Previously create_link / create_deep_link allowed an anonymous caller
-- (p_user_id = NULL, auth.uid() = NULL) to create ownerless links. This
-- migration removes anonymous creation entirely: both RPCs now reject any
-- caller without a session. Since these SECURITY DEFINER functions are the
-- ONLY write path to links for anon/authenticated roles (direct INSERT is
-- blocked by RLS), this is the authoritative enforcement — a raw REST call
-- with the anon key is rejected too, not just the UI.
--
-- The inserted user_id is always the authenticated caller (COALESCE to
-- auth.uid()), so a link can never be created unowned or on someone else's
-- behalf regardless of what the client passes for p_user_id.

-- ============================================================================
-- 1. create_link — auth required
-- ============================================================================
...
BEGIN
  ...
  IF auth.uid() IS NULL THEN
    RAISE EXCEPTION 'You must be signed in to create links';
  END IF;
  ...
  -- Ownership guard: a supplied user id must be the caller's own.
  ...
  VALUES (p_short_code, p_original_url, COALESCE(p_user_id, auth.uid()), p_original_url)
...
-- ============================================================================
-- 2. create_deep_link — auth required
-- ============================================================================
...
BEGIN
  ...
  -- Reject dangerous deep-link schemes (case-insensitive).
  ...
  VALUES (
    p_short_code, p_original_url, COALESCE(p_user_id, auth.uid()), p_original_url,
